...
<head>
<meta charset="UTF-8">
<title>EveReet - 회원정보조회(R4,R41)</title>
</head>
<body>
<h1>EveReet - 회원정보조회(R4,R41)</h1>
<hr>
<%
	MemberDTO member;
...
<a href="./getMemberListForPage.sh?currentPageNo=0">[FIRST]</a>
<%
	if( currentPageNo > 0){		
%>
	<a href="./getMemberListForPage.sh?currentPageNo=<%=(currentPageNo-1) %>">[PRE]</a>
<%
	}else{
%>	
	[PRE]
...
%>
			<a href="./getMemberListForPage.sh?currentPageNo=<%=i%>">[<%=(i+1)%>]</a>
<%
		}
	}
%>
...
%>
	<a href="./getMemberListForPage.sh?currentPageNo=<%=(currentPageNo+1)%>">[NXT]</a>
<%
	}else{
%>
	[NXT]	
...
%>
 	<a href="./getMemberListForPage.sh?currentPageNo=<%=(bpiVO.getPageCnt()-1)%>">[END]</a>
